...
interface Rule {
  ...
  priority: Priority;
  suggested_agent: string;
  extractContext: (match: RegExpMatchArray, fullText: string) => Partial<TaskContext>;
  generatePrompt: (context: Partial<TaskContext>, match: RegExpMatchArray) => string;
}
...
const RULES: Rule[] = [
  // Test failure - FAIL pattern
  {
    ...
    generatePrompt: (ctx) =>
      `Fix the failing test in ${ctx.test_file}. Analyze the test failure and implement the necessary fix.`,
  },
  ...
  // Test failure - generic
  {
    ...
    generatePrompt: (ctx) =>
      `Fix the failing test: "${ctx.test_name}". Analyze the test failure and implement the necessary fix.`,
  },
  ...
  // Agent stuck - timeout
  {
    ...
    generatePrompt: () =>
      `Investigate the command timeout. Check if the process is hung or if there's an infinite loop.`,
  },
  ...
  // Agent stuck - no output
  {
    ...
    generatePrompt: () =>
      `Investigate why there's no output. The process may be stuck or waiting for input.`,
  },
  ...
  // CI failure - emoji
  {
    ...
    generatePrompt: (ctx) =>
      `CI pipeline failed. Investigate the failure: ${ctx.log_excerpt}`,
  },
  ...
  // CI failure - text
  {
    ...
    generatePrompt: () =>
      `CI/Build failed. Investigate the failure and fix the underlying issue.`,
  },
  ...
  // PR check failed
  {
    ...
    generatePrompt: () =>
      `PR check failed. Review the check failure and address the issues.`,
  },
  ...
  // Push failed
  {
    ...
    generatePrompt: () =>
      `Git push failed. Check for remote conflicts or permission issues.`,
  },
  ...
  // Generic Error: pattern (no stack trace)
  {
    ...
    generatePrompt: (ctx) =>
      `Investigate and fix the error: ${ctx.log_excerpt}`,
  },
];
...
export class RuleEngine {
  ...
  constructor(loopThreshold: number = 5) {
   ...
   */
  private detectLoop(log: string): RuleMatch | null {
