...
import { checkNode, decidePreflight, type NodeCheck } from '../services/node-guard.js';
import * as startProxyModule from './start-proxy.js';

/** Injected at bundle time by esbuild `define` (scripts/bundle-cli.js). Undefined under tsc/ESM. */
declare const __OSS_LAUNCH_VERSION__: string | undefined;
/** True ONLY in the esbuild bundle that the pkg binary wraps; undefined under tsc/ESM (node-script). */
declare const __OSS_BUNDLED__: boolean | undefined;
...
export function resolveEntry(argv: string[]): 'start-proxy' | 'launch' {
...
export async function runLaunch(argv: string[], deps: LaunchDeps): Promise<number> {
  ...
  const nodeCheck = (deps.nodeCheck ?? (() => ({ ok: true } as NodeCheck)))();
  ...
  if (decision.useProxy && preflight.route && decision.port !== undefined) {
    ...
    (deps.warn ?? ((m: string) => console.error(m)))(preflight.banner ?? '');
...
export async function main(argv: string[]): Promise<number | undefined> {
  ...
  if (isVersionRequest(argv)) {
    let version =
      typeof __OSS_LAUNCH_VERSION__ !== 'undefined' && __OSS_LAUNCH_VERSION__ ? __OSS_LAUNCH_VERSION__ : 'unknown';
    if (version === 'unknown') {
      try {
