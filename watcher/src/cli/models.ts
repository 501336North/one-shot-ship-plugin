...
import { isValidModelId, SUPPORTED_PROVIDERS, Provider } from '../types/model-settings.js';
 ...
 */
export function setTestModeProviderFail(fail: boolean): void {
...
/**
 * Model registry - list of available models grouped by provider
 */
interface ModelInfo {
  id: string;
  name: string;
 ...
 */
function detectPromptType(promptName: string): 'agent' | 'command' | 'skill' | 'hook' {
...
function listModels(): string {
  ...
  const providers = ['openrouter', 'ollama', 'openai', 'gemini'] as const;
 ...
 */
function searchModels(query: string | null, freeOnly: boolean): string {
  const lines: string[] = [];
...
function showConfig(): string {
  ...
  const defaultModel = (models?.default as string) || 'claude';
  lines.push(`Default: ${defaultModel}`);
  lines.push('');

  const types = ['agents', 'commands', 'skills', 'hooks'] as const;
  let hasCustom = false;

  for (const type of types) {
    const mappings = models?.[type] as Record<string, string> | undefined;
 ...
 */
function setModel(promptName: string, modelId: string): string {
  ...
  const typeKey = `${promptType}s` as const; // agents, commands, skills, hooks

  if (!models[typeKey]) {
    models[typeKey] = {};
  }

  (models[typeKey] as Record<string, string>)[promptName] = modelId;
 ...
 */
function handleKeys(args: string[]): string {
 ...
 */
function setKey(provider: string, key: string): string {
  // Validate provider
  ...
  (config.apiKeys as Record<string, string>)[provider] = key;
 ...
 */
export async function executeModelsCommand(args: string[]): Promise<string> {
  if (args.length === 0) {
    return showHelp();
  }
