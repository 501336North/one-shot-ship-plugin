...
export async function checkGitSafety(): Promise<CheckResult> {
  try {
    ...
    if (isProtectedBranch) {
      ...
      let lastAgentMerge: string | null = null;
      try {
        // Find the most recent agent commit that was properly merged via PR
        const { stdout: lastMergedAgent } = await execAsync(
          'git log main --grep="Co-Authored-By: Claude" --grep="(#" --all-match --format="%ar" -1 2>/dev/null || true'
        );
