...
export class TelegramService {
  ...
  async sendMessage(
    ...
    if (!response.ok) {
      const error = (await response.json()) as TelegramApiResponse;
      throw new Error(`Telegram API error: ${error.description || response.status}`);
    }

    const data = (await response.json()) as TelegramApiResponse;
    const result = data.result as TelegramSendMessageResult;
    ...
    while (true) {
      ...
      try {
        const response = await fetch(
          `${this.baseUrl}/getUpdates?offset=${lastUpdateId + 1}&timeout=5`,
          { method: 'GET' }
        );

        if (!response.ok) continue;

        const data = (await response.json()) as TelegramGetUpdatesResult;
  ...
  async validateConfig(): Promise<{
    ...
    try {
      ...
      const meData = (await meResponse.json()) as TelegramApiResponse;
      const meResult = meData.result as TelegramGetMeResult | undefined;
    ...
    try {
      ...
      const testData = (await testResponse.json()) as TelegramApiResponse;
      const testResult = testData.result as TelegramSendMessageResult;
      canSendToChat = true;
