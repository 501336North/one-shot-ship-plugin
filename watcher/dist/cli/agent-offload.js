...
import { checkAgentModel } from './agent-model-check.js';
/**
 ...
 */
export function buildOffloadInvocation(routing) {
    return {
        cmd: 'claude',
        ...
        env: stripModelEnv({
            ...process.env,
            ANTHROPIC_BASE_URL: routing.proxyUrl,
...
export function classifyResult(exitCode, stdout) {
    if (exitCode !== 0)
        return 'spawn_error';
...
function defaultIsProxyReachable(url) {
 ...
 */
function defaultStartProxy(opts) {
    return new Promise((resolve) => {
        try {
            ...
            const child = childProcess.spawn('node', [script, '--model', opts.model, '--port', String(opts.port)], { detached: true, stdio: 'ignore' });
            child.unref();
        }
        catch {
 ...
 */
export async function runAgentOffload(params, deps = {}) {
    ...
    const readPrompt = deps.readPrompt ?? ((f) => fs.readFileSync(f, 'utf8'));
    ...
    try {
        ...
        if (!(await isProxyReachable(routing.proxyUrl))) {
            ...
            if (!up)
                return degrade('proxy_down', fallbackEnabled);
        }
        ...
        return degrade('spawn_error', fallbackEnabled, err?.message);
    }
}
// --- CLI entrypoint ---------------------------------------------------------
export function parseOffloadArgs(argv) {
    let agent = '';
    let promptFile = '';
    let projectDir;
...
if (invokedDirectly) {
    ...
    if (!params.agent || !params.promptFile) {
        process.stdout.write(JSON.stringify({ offloaded: false, reason: 'native', error: '--agent and --prompt-file required' }));
        process.exit(0);
    }
...
//# sourceMappingURL=agent-offload.js.map