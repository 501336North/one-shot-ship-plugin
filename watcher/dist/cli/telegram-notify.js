 ...
 */
export function parseButtons(json) {
    if (!json) {
        return [];
    }
    try {
 ...
 */
export async function sendTelegramNotification(message, buttons) {
 ...
 */
export async function waitForCallback(messageId, options = {}) {
    const settings = loadTelegramSettings();
    ...
    while (true) {
        ...
        try {
            const response = await fetch(`${baseUrl}/getUpdates?offset=${lastUpdateId + 1}&timeout=5`, { method: 'GET' });
            if (!response.ok)
                continue;
            const data = await response.json();
