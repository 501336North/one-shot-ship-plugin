...
export async function setupTelegram(botToken) {
    ...
    try {
        await fullService.sendMessage('OSS Dev Workflow connected successfully!');
    }
    catch (error) {
        return {
            success: false,
 ...
 */
export function formatSetupOutput(result) {
    const lines = [];
    ...
    if (result.step === 'fetch_chat' && !result.success) {
        lines.push(`X ${result.message}`);
        lines.push('');
        lines.push('Please send any message to your bot in Telegram, then run this command again.');
        return lines.join('\n');
    }
...
//# sourceMappingURL=telegram-setup.js.map