...
export class WorkflowAnalyzer {
     ...
     */
    analyze(entries, now = new Date(), anchors) {
        ...
        for (const entry of entries) {
            ...
            if (entry.event === 'START') {
                ...
                if (entry.cmd in chainProgress) {
                    chainProgress[entry.cmd] = 'in_progress';
                }
            }
            if (entry.event === 'COMPLETE') {
                commandComplete = true;
                if (entry.cmd in chainProgress) {
                    chainProgress[entry.cmd] = 'complete';
                }
            }
            if (entry.event === 'FAILED') {
                if (entry.cmd in chainProgress) {
                    chainProgress[entry.cmd] = 'failed';
                }
            }
            ...
            if (entry.event === 'AGENT_SPAWN') {
                const agentId = entry.data.agent_id;
                const agentType = entry.data.agent_type;
                activeAgents.push({
                    id: agentId,
                    type: agentType,
                    spawn_time: entry.ts,
    ...
    detectStuckPhase(state, now, issues) {
        if (!state.phaseStartTime || state.phaseComplete)
            return;
    ...
    detectRegression(entries, issues, anchors) {
        ...
        let completedPhase = anchorCompleted.length > 0 ? anchorCompleted[anchorCompleted.length - 1] : undefined;
        for (const entry of entries) {
            if (entry.event === 'PHASE_COMPLETE') {
                hadComplete = true;
    ...
    detectOutOfOrder(entries, issues, anchors) {
        const seenPhases = [];
        for (const entry of entries) {
            if (entry.event === 'PHASE_START' && entry.phase) {
    ...
    detectChainViolation(entries, state, issues, anchors) {
        ...
        const hasPrerequisite = prerequisites.some((prereq) => state.chainProgress[prereq] === 'complete' ||
            anchors?.chainProgress[prereq] === 'complete' ||
    ...
    detectTddViolation(entries, issues) {
    ...
    detectExplicitFailures(entries, issues) {
        for (const entry of entries) {
            if (entry.event === 'FAILED') {
                issues.push({
    ...
    detectAgentFailures(entries, issues) {
        for (const entry of entries) {
    ...
    detectSilence(state, now, issues) {
        ...
        if (!state.currentCommand && !state.currentPhase)
            return;
    ...
    detectMissingMilestones(entries, state, issues) {
    ...
    detectDecliningVelocity(state, _now, issues) {
        if (state.milestoneTimestamps.length < 4)
            return;
    ...
    detectIncompleteOutputs(entries, issues) {
        for (const entry of entries) {
            if (entry.event === 'COMPLETE') {
    ...
    detectAgentSilence(state, now, issues) {
    ...
    detectAbruptStop(state, now, issues) {
        if (!state.lastActivityTime || state.commandComplete)
            return;
        const elapsed = now.getTime() - new Date(state.lastActivityTime).getTime();
    ...
    detectPartialCompletion(state, now, issues) {
        if (state.commandComplete)
            return;
        ...
        if (!hasCompletedPhases || !state.phaseStartTime)
            return;
        const elapsed = now.getTime() - new Date(state.phaseStartTime).getTime();
    ...
    detectAbandonedAgent(state, now, issues) {
        for (const agent of state.activeAgents) {
            if (agent.started && !agent.completed) {
                const elapsed = now.getTime() - new Date(agent.spawn_time).getTime();
