...
export class MetricsCollector {
    ...
    getDailyMetrics() {
        ...
        const successCommands = todaySessions.reduce((sum, s) => sum + s.commands.filter(c => c.status === 'success').length, 0);
        return {
            date: today,
    ...
    getAllTimeMetrics() {
        ...
        const totalSuccesses = sessions.reduce((sum, s) => sum + s.commands.filter(c => c.status === 'success').length, 0);
