...
export class SpecMetricsService {
    ...
    async updateCoverage(feature, metrics) {
        ...
        if (featureData.history.length > MAX_HISTORY_ENTRIES) {
            featureData.history = featureData.history.slice(featureData.history.length - MAX_HISTORY_ENTRIES);
        }
     ...
     */
    async calculateVelocity(feature) {
        ...
        const firstHalfAvg = firstHalf.reduce((sum, entry) => sum + entry.drift_count, 0) / firstHalf.length;
        const secondHalfAvg = secondHalf.reduce((sum, entry) => sum + entry.drift_count, 0) / secondHalf.length;
     ...
     */
    async recordReconciliation(entry) {
        const metrics = await this.loadMetrics();
        // Add new entry
        ...
        if (metrics.reconciliations.length > MAX_RECONCILIATION_ENTRIES) {
            metrics.reconciliations = metrics.reconciliations.slice(metrics.reconciliations.length - MAX_RECONCILIATION_ENTRIES);
        }
        await this.saveMetrics(metrics);
    }
}
//# sourceMappingURL=spec-metrics.js.map