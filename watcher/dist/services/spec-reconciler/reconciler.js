 ...
 */
export async function classifyDrift(drift, searchPaths) {
...
export class SpecReconciler {
     ...
     */
    async queueDriftTask(drift, feature, specPath) {
     ...
     */
    async reconcile(drifts, feature, specPath, searchPaths) {
        const entries = [];
        let fixed = 0;
        let queued = 0;
        let failed = 0;
     ...
     */
    async attemptAutoFix(drift, specPath) {
