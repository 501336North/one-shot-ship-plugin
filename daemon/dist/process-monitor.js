...
export class ProcessMonitor {
     ...
     */
    async findProcesses(search) {
        try {
            // Use ps to find processes
            // Format: pid,etime,pcpu,rss,args
            const output = execSync(`ps aux | grep -i "${search}" | grep -v grep`, { encoding: 'utf-8', timeout: 5000 }).trim();
            if (!output) {
                return [];
            }
     ...
     */
    isProcessHung(process, timeoutMs) {
