...
```bash
~/.oss/bin/oss-decrypt --type agents --name frontend-design
```

## Step 4: Execute the Fetched Prompt

The fetched prompt provides the frontend-design agent with full design intelligence for elevating UI quality — visual review, design system compliance, accessibility checks, and interaction refinement.
