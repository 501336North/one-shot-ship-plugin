...
After `oss-notify.sh --workflow plan complete` runs, its stdout may contain a `---CHAIN_COMMANDS---` block. If present, **auto-execute each `CHAIN:` line in order** — no user intervention required.
...
- Log each: `~/.oss/hooks/oss-log.sh write plan "[CHAIN] executing /oss:<command>"`
